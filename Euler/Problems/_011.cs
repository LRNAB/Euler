...
namespace Euler.Problems
{
    ...
    public class _011 : IProblem
    {
        ...
        // No better implementation than http://www.geekality.net/2009/10/03/project-euler-problem-11/
        public string Solve()
        {
            ...
            for (var row = 0; row < rows; row++)
            {
                for (var column = 0; column < columns; column++)
                {
                    ...
                    {
                        // Right and Left
                        greatestProduct = Math.Max(greatestProduct, _grid[row, column] * _grid[row, column + 1] * _grid[row, column + 2] * _grid[row, column + 3]);
                    }

                    if (row < rows - 3)
                    {
                        // Down and Up
                        greatestProduct = Math.Max(greatestProduct, _grid[row, column] * _grid[row + 1, column] * _grid[row + 2, column] * _grid[row + 3, column]);

                        // Diagonally, down to the right
                        if (column < columns - 3)
                            greatestProduct = Math.Max(greatestProduct, _grid[row, column] * _grid[row + 1, column + 1] * _grid[row + 2, column + 2] * _grid[row + 3, column + 3]);
            ...
            return String.Format("Greatest product of four adjacent numbers in the same direction in the 20×20 grid is {0}", greatestProduct);
        }
    }
}